...
public class MailUtil {
    ...
    public static String renderMailHtmlContent(HashMap map, String templateId) throws Exception {
        ...
        String templatePath = APSystem.getSysDir() + APSystem.getProjectParam("MAIL_TEMP_PATH");
        ...
        try {
			...
			parm = home.fetchSysParmByKey("AD_URL");
			
			if (!StringUtil.isBlank(parm)) {
				adUrl = parm.PARM_VALUE;
			}
		} catch (Exception e) {
			LOG.error("Failed to fetch sysParam!!", e);
			throw e;
        ...
        context.put("WebUrl", APSystem.getProjectParam("BANK_WEB_URL"));
        context.put("CustomerService", APSystem.getProjectParam("CUST_SRV_TEL"));
        context.put("EcgwUrl", APSystem.getProjectParam("ECGW_URL"));
        
        //至SYS_PARM 取得 AD_MAIL 的參數值
        context.put("AdUrl", adUrl);
    ...
    public void addMail(String content, String subject, String receivers, 
        ...
        mailNotc.setMailServer(APSystem.getProjectParam("MAIL_SERVER"));
        mailNotc.setMailServerPort(APSystem.getProjectParam("MAIL_SERVER_PORT"));
        mailNotc.setSender(APSystem.getProjectParam("MAIL_SENDER"));
        mailNotc.setMailUser(APSystem.getProjectParam("MAIL_USER"));
        mailNotc.setAuthentic(APSystem.getProjectParam("AUTHENTIC").equals("1"));
        mailNotc.setMailPswd(APSystem.getProjectParam("MAIL_PSWD"));
        mailNotc.setNickName(APSystem.getProjectParam("NICKNAME"));
        mailNotc.setMailFrom(APSystem.getProjectParam("MAIL_FROM"));
        ...
        +"/Sender:"+APSystem.getProjectParam("MAIL_SENDER")
        +"/MailUser:"+mailNotc.getMailUser()
        +"/Authentic:"+APSystem.getProjectParam("AUTHENTIC")
        +"/NickName:"+APSystem.getProjectParam("NICKNAME")
        +"/MailFrom:"+APSystem.getProjectParam("MAIL_FROM")); //v1.01

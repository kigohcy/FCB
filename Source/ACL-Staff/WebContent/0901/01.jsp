	...
	$("#btnOk").click(function() {
			if ($("#form1").valid()) {
				if(confirm("<fmt:message key="message.cnfm.insertOrNot" />")){
					$("#form1").attr("action", root + "/0901/roleAdd.html");
					$("#form1").submit();
				}
			}
		});
...
<%-- 表單驗證邏輯 --%>
	$("#form1").validate({
			...
			messages : {
				roleId : {
					required : "<fmt:message key="message.alert.roleId" /><fmt:message key="message.alert.notNull" />",
					ALPHA_CHECKER : "<fmt:message key="F0901.field.Role_Code" /><fmt:message key="message.alert.onlyEN&NUM" />"
				},
				roleName : {
				...
				<form id="form1" name="form1" action="" method="post">
					<table class="fxdTable" width="100%">
						...
						<tr class="dataRowOdd">
							...
							<td width="150px;">
								<input name="roleId" type="text" maxlength="10" />
							</td>
							<td width="100px;">
								<%-- 角色名稱  --%>
							...
							<td width="150px;">
								<input name="roleName" type="text" maxlength="40" />
							</td>
						</tr>
						<tr class="secondaryTitleRow">
							<td colspan="4">
						...
						<c:forEach items="${command.staffSysMenu}" var="menu">
						    <c:if test="${not empty menu.fncts }">
								...
								<c:forEach items="${menu.fncts}" var="fnct" varStatus="i">
									...
									<td>
										<input type="checkbox" name="fnctIds" value='<c:out value="${fnct.id.fnctId}"/>' />
										<%-- 功能名稱 --%>

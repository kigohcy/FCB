		...
		$("#btnOk").click(function() {
			if ($("#form1").valid()) {
				if(confirm("<fmt:message key="message.cnfm.updateOrNot" />")){
					$("#form1").attr("action", root + "/0904/userUpdate.html");
					$("#form1").submit();
				}
			}
		});
				...
				<form id="form1" name="form1" action="" method="post">
					<table id="datatable" class="vTable" width="980px;">
						...
						<tr class="dataRowEven">
							...
							<%--員工編號* --%>
							<td>
								<input type="text" size="10" maxlength="10" name="userId" id="userId" value="<c:out value="${command.userId}" />"/>
							</td>
						</tr>
						...
						<tr class="dataRowOdd">
							...
							<%--姓名 * --%>
							<td>
								<input type="text" size="20" maxlength="20" name="userName" id="userName" value="<c:out value="${command.userName}" />" />
							</td>
						</tr>
						<tr class="dataRowEven">
							...
							<%-- 登入代號* --%>
							<td>
								<input disabled="disabled" type="text" size="20" maxlength="20" name="loginId" id="loginId" value="<c:out value="${command.loginId}" />" />
							</td>
						</tr>
						<tr class="dataRowOdd">
							...
							<%--角色權限別--%>
							<td>
								<c:if test="${command.roleId != 'ADMIN' }">
								...
								<c:if test="${command.roleId == 'ADMIN' }">
									${command.roleId}
									<input type="hidden" id="roleId" name="roleId" value="${command.roleId}" />
								</c:if>
							</td>
						</tr>
						<tr class="dataRowEven">
							...
							<%--EMAIL  --%>
							<td>
								<input type="text" size="40" maxlength="40" name="email" id="email" value="<c:out value="${command.email}" />"/>
							</td>
						</tr>
						<tr class="dataRowOdd">
							<th><fmt:message key="F0904.field.STATES" /></th>
							<%--狀態--%>
							<td>
								<select name="states" id="states" style="width:150px;">
									<c:forEach items="${command.statesList}" var="states">        
										<option value="<c:out value="${states.userStates}" />" <c:if test="${states.userStates eq command.states }">selected</c:if> ><c:out value="${states.statesName}" />&nbsp;
										</option>
									</c:forEach>
								</select>
							</td>

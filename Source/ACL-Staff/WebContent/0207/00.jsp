...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#form1").validate({
		         ...
		         messages:{
		        	 custId:{
		        		 //請輸入身分證字號
		        		 required:"<fmt:message key="message.alert.pleaseKeyIn" />"+"<fmt:message key="F0207.field.custId" />",
		...
		function send(){
			if($("#form1").valid()){ // form validate
				$("#form1").attr("action", root + "/0207/query.html");
				$("#form1").submit();
			}
		}
	</script>
	...
	<div class="container">
		...
		<div class="mainContent">
			<div class="content">
				<div class="fnctTitle"><fmt:message key="function.Id.F0207" /><c:if test="${not command.initQuery}"> > <fmt:message key="common.btn.Query" /></c:if> </div> <%--服務狀態查詢  > 查詢--%>
				<div id="accordion">
					...
					<div style="width: 978px;">
						<form method="post" name="form" id="form1" action="#" style="margin: 0;">
							<table class="fxdTable" width="100%">
								<tr class="dataRowOdd">
									<td width="90px"><fmt:message key="F0207.field.custId" />*</td><%--身分證字號* --%>
									<td>
						...
						<c:when test="${not empty command.custData}">
							<table class="fxdTable" width="980px">
								<tr class="secondaryTitleRow">
									<td colspan="6"><fmt:message key="F0207.field.custId" /><%--身分證字號 --%>：${command.custData.custId} ${command.custData.custName}</td>
								</tr>
							</table>
							<p></p>
							<c:choose>
								...
								<div class="abgne_tab">
									<ul class="tabs">
										<li><a href="#showCustAcnt"><fmt:message key="F0207.field.bankAcntLimt" /></a></li><%--銀行存款帳號交易限額 --%>
									    <c:forEach items="${command.custPltfList}" var="item">
									    	<li><a href="#${item.id.ecId}"><fmt:message key="F0207.field.tagCustomLimt"/> - <c:out value="${item.ecNameCh}" /></a></li>
									    </c:forEach>
									</ul>
									<div class="tab_container">
										<div id="showCustAcnt" class="tab_content">
											<table class="fxdTable" width="920px">
												<tr class="titleRow">
													<td align="left" colspan="6">&nbsp;<fmt:message key="F0207.field.bankAcntLimt" /></td><%--銀行存款帳號交易限額 --%>
												</tr>
												<tr class="titleRow">
													<td width="13%;" nowrap><fmt:message key="F0207.field.realAcnt" /></td><%--實體帳號 --%>
													<td width="13%;" nowrap><fmt:message key="F0207.field.trnsLimt" />(NT$)</td><%--每筆交易限額(NT$) --%>
													<td width="13%;" nowrap><fmt:message key="F0207.field.dayLimt" />(NT$)</td><%--每日交易限額(NT$) --%>
													<td width="13%;" nowrap><fmt:message key="F0207.field.mnthLimt" />(NT$)</td><%--每月交易限額(NT$) --%>
													<td width="13%;" nowrap><fmt:message key="F0207.field.availableBalanceDay" />(NT$)</td><%--每日剩餘可用餘額(NT$) --%>
													<td width="13%;" nowrap><fmt:message key="F0207.field.availableBalanceMonth" />(NT$)</td><%--每月剩餘可用餘額(NT$) --%>
												</tr>
												<c:choose>
													<c:when test="${not empty command.custAcntList}">
														<c:forEach items="${command.custAcntList}" var="custAcnt" varStatus="theCount">
															...
															<tr class="${cssClass}">
																...
																<td align="right">
																	<c:choose>
																		<c:when test="${custAcnt.trnsLimt eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
																		<c:otherwise><fmt:formatNumber type="number" pattern="###,###,###" value="${custAcnt.trnsLimt}" /></c:otherwise>
																	</c:choose>
																</td>
																<td align="right">
																	<c:choose>
																		<c:when test="${custAcnt.dayLimt eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
																		<c:otherwise><fmt:formatNumber type="number" pattern="###,###,###" value="${custAcnt.dayLimt}" /></c:otherwise>
																	</c:choose>
																</td>
																<td align="right">
																	<c:choose>
																		<c:when test="${custAcnt.mnthLimt eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
										 ...
										 <c:forEach items="${command.custPltfList}" var="item">
										 	<div id="${item.id.ecId}" class="tab_content">
												...
												<table class="fxdTable" width="920px">
													<tr class="titleRow">
														<td align="left" colspan="13"><fmt:message key="F0207.field.tagCustomLimt"/> - <c:out value="${item.ecNameCh}" /></td><%--平台連結帳號交易限額 --%>
													</tr>
													<tr class="titleRow">
														<td rowspan="2" width="10%;" nowrap><fmt:message key="F0207.field.ecUser" /></td><%--平台會員代號 --%>
														<td rowspan="2" width="13%;" nowrap><fmt:message key="F0207.field.realAcnt" /></td><%--實體帳號 --%>
														<td rowspan="2" width="9%;" nowrap><fmt:message key="F0207.field.gradeType" /></td><%--身分認證 --%>
														<td rowspan="2" width="5%;"  nowrap><fmt:message key="F0207.field.grade" /></td><%--等級 --%>
														<td rowspan="2" width="5%;"  nowrap><fmt:message key="F0207.field.stts" /></td><%--狀態 --%>
														<td colspan="2" width="13%;" nowrap><fmt:message key="F0207.field.trnsLimt" />(NT$)</td><%--每筆交易限額(NT$) --%>
														<td colspan="3" width="20%;" nowrap><fmt:message key="F0207.field.dayLimt" />(NT$)</td><%--每日交易限額(NT$) --%>
														<td colspan="3" width="20%;" nowrap><fmt:message key="F0207.field.mnthLimt" />(NT$)</td><%--每月交易限額(NT$) --%>
													</tr>
													<tr class="titleRow">
														<td><fmt:message key="F0207.field.legalLimt" /></td><%--法定限額 --%>
														<td><fmt:message key="F0207.field.customLimt" /></td><%--自訂限額 --%>
														<td><fmt:message key="F0207.field.legalLimt" /></td><%--法定限額 --%>
														<td><fmt:message key="F0207.field.customLimt" /></td><%--自訂限額 --%>
														<td><fmt:message key="F0207.field.availableBalance" /></td><%--可用餘額 --%>
														<td><fmt:message key="F0207.field.legalLimt" /></td><%--法定限額--%>
														<td><fmt:message key="F0207.field.customLimt" /></td><%--自訂限額 --%>
														<td><fmt:message key="F0207.field.availableBalance" /></td><%--可用餘額 --%>
													</tr>
														...
														<c:when test="${not empty command.custAcntLink[key]}">
															<c:forEach items="${command.custAcntLink[key]}" var="acnt" varStatus="theCount">
																...
																<tr class="${cssClass}">
																	...
																	<td align="center"><fmt:message key="F0207.field.queryType.${acnt.gradType}"/></td>
																	<td align="center">${acnt.grad}</td>
																	<td align="center"><fmt:message key="F0207.field.stts.${acnt.stts}"/></td>
																	<td align="right">
																		<c:choose>
																			<c:when test="${command.baseLimt[grad].trnsLimt  eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
																			<c:otherwise>&lt;<fmt:formatNumber  type="number" pattern="###,###,###"   value="${command.baseLimt[grad].trnsLimt}" /></c:otherwise>
																		</c:choose>
																	</td>
																	<td align="right">
																		<c:choose>
																			<c:when test="${acnt.trnsLimt eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
																			<c:otherwise><fmt:formatNumber type="number" pattern="###,###,###" value="${acnt.trnsLimt}" /></c:otherwise>
																		</c:choose>
																	</td>
																	<td align="right">
																		<c:choose>
																			<c:when test="${command.baseLimt[grad].dayLimt  eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
																			<c:otherwise>&lt;<fmt:formatNumber  type="number" pattern="###,###,###"   value="${command.baseLimt[grad].dayLimt}" /></c:otherwise>
																		</c:choose>
																	</td>
																	<td align="right">
																		<c:choose>
																			<c:when test="${acnt.dayLimt eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
																	...
																	<td align="right">
																		<c:choose>
																			<c:when test="${command.baseLimt[grad].mnthLimt  eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>
																			<c:otherwise>&lt;<fmt:formatNumber  type="number" pattern="###,###,###"   value="${command.baseLimt[grad].mnthLimt}" /></c:otherwise>
																		</c:choose>
																	</td>
																	<td align="right">
																		<c:choose>
																			<c:when test="${acnt.mnthLimt eq '0'}"><fmt:message key="F0207.field.noLimt" /></c:when><%--無限額 --%>

			...
			<div class="content">
				...
				<div id="before" style="height: 350px; width: 485px; overflow-y: scroll; overflow-x: scroll; float: left;">
					<table id="datatable1" class="vTable" width="980px">
			            ...
			            <c:choose>
					        <c:when test="${command.q_action =='S' or command.q_action =='D'}">
					        	<c:if test="${command.q_action =='D'}"> <%--刪除--%>
					        		<%@include file="F0201D_B.jsp"%>
			                    </c:if>
			                    <c:if test="${command.q_action =='S'}"> <%--變更--%>
			                        <%@include file="F0201_B.jsp"%>
			                    </c:if>
						    </c:when>
						    <c:otherwise>
						    	<tr class="dataRowOdd">
	           ...
	           <div id="after" style="height: 350px; width: 485px; overflow-y: scroll; overflow-x: scroll; float: right;">
	               	<table id="datatable2" class="vTable" width="980px">
						<tr class="staffFnctTitleRow">
			            	<td colspan="2"><fmt:message key="common.operate.afterMdfy"/></td><%--異動前 --%>
			            </tr>
		            	<c:choose>
					        <c:when test="${command.q_action =='S'}">
			                      <%@include file="F0201_A.jsp"%>
						    </c:when>
						    <c:otherwise>
						    	<tr class="dataRowOdd">
						    		<td colspan="2"><fmt:message key="common.operate.haveNoData"/> <%--無資料 --%></td>

...
<%
	 ...
	 *
	 * Description: 操作記錄查詢 - 扣款平台管理覆核異動前
	 *
	 * Modify History:
...
<fmt:formatDate value="${item.id.cretDttm}" var="cretDttm" pattern="yyyy-MM-dd HH:mm:ss.SSS" />
	<c:choose>
		<c:when
			test="${item.id.ecId eq keys[0] && item.id.cretUser eq keys[1] && cretDttm eq keys[2]}">
			<tr class="dataRowOdd">
				<th><fmt:message key="F0201.field.EC_ID" />*</th><%--平台代號* --%>
				<td>${item.id.ecId} - <fmt:message key="F0201.field.DATA_STTS.${item.dataStts}" /></td>
			</tr>
			...
			<tr class="dataRowOdd">
				...
				<td><input type="text" size="20" value="${item.ecNameEn}"
					readonly /></td>
			</tr>
			<tr class="dataRowEven">
				<th><fmt:message key="F0201.field.FEE_TYPE" />*</th><%-- 收費方式* --%>
				<td><input type="radio" value="A"
					<c:if test="${item.feeType eq 'A'}">checked</c:if> disabled /> <fmt:message
						key="F0201.field.FEE_TYPE.A" />&nbsp; <input type="radio"
					value="B" <c:if test="${item.feeType eq 'B'}">checked</c:if>
					disabled /> <fmt:message key="F0201.field.FEE_TYPE.B" />&nbsp;</td>
			</tr>
			<tr class="dataRowOdd">

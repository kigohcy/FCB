...
<%-- 確定 --%>
	$("#btnOK").click(function() {
			if($("#form1").valid()){
				$("#form1").attr("action", root + "/0401/modify.html");
				$("#form1").submit();
			}
		});
<%-- 取消 --%>
...
<%-- 選取該筆公告在DB中存取的公告類型 --%>
	...
	function selectType() {
		var rtnType = "<c:out value="${command.type}"/>"
		...
		<div class="mainContent">
			<div class="content">
				<div class="fnctTitle">
					...
					<c:out value=">" />
					<fmt:message key="F0401.field.modify" />
				</div>
				<form action="#" id="form1" method="post">
					<table class="fxdTable" width="100%">
						<tr class="secondaryTitleRow">
							<td colspan="2">
								<%--  修改公告 --%>
								<fmt:message key="F0401.field.modifyNews" />
							</td>
						</tr>
						<tr class="dataRowOdd">
							<td width="100px">
							...
							<td>
								<input type="text" maxlength="10" name="aBgnDate" datePicker="true" value="<fmt:formatDate pattern="yyyy/MM/dd" value="${command.bgnDate}" />" />
								~
								<input type="text" maxlength="10" name="aEndDate" datePicker="true" value="<fmt:formatDate pattern="yyyy/MM/dd" value="${command.endDate}" />" />
							</td>
						</tr>
						...
						<tr class="dataRowOdd">
							...
							<td>
								<c:choose>
									<c:when test="${command.serl eq 99}">
									...
									<c:otherwise>
										<input type="radio" name="serl" value="99">
										<%-- 否 --%>
										<fmt:message key="F0401.field.serl.N" />
										<input type="radio" name="serl" value="1" checked="checked">
										<%-- 是 --%>
										<fmt:message key="F0401.field.serl.Y" />
									</c:otherwise>
								</c:choose>
						...
						<tr class="dataRowEven">
							...
							<td>
								<input type="text" size="50" name="qTitle" value="${command.title }" maxlength="50"/>
						...
						<tr class="dataRowOdd">
							...
							<td>
								<textarea name="content">${command.content }</textarea>
							</td>
						</tr>
					</table>

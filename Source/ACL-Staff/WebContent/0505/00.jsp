...
<%--查詢--%>
	$("#btnQuery").click(function() {
			if ($("#form1").valid()) {
				$("#form1").attr("action", root + "/0505/query.html");
				$("#form1").submit();
			}
		});
<%--明細資料查詢--%>
	$("#btnDetail").click(function() {
			$("#form2").append("<input type='hidden' name='dStts' value=''/>").attr(
			                   "action", root + "/0505/queryDetail.html");
			$("#form2").submit();
		});
...
<%--超連結明細資料查詢--%>
	$(".trnsItem").click(
				function() {
					var trnsItem = $(this);
					$("#form2").append("<input type='hidden' name='dStts' value='" + trnsItem.attr("stts") + "'/>").attr("action",
							root + "/0505/queryDetail.html").submit();
				});

<%--Excel檔案下載--%>
	$("#btnDownload").click(function() {
			$("#form2").append("<input type='hidden' name='templateName' value=''/>").attr(
			                   "action", root + "/0505/download.html");
			$("#form2").submit();
		});
						...
						<form method="post" name="form1" id="form1" action="" style="margin: 0;">
							<table class="fxdTable" style="width: 100%;">
								<tr class="dataRowOdd">
									<td width="150px">
										<%--查詢日起訖--%>
										<fmt:message key="F0505.field.queryDate" />
										*
									</td>
									<td>
								...
								<tr class="dataRowEven">
									<td>
										<fmt:message key="F0505.field.reportType" />
									</td>
									<td>
							...
							<c:if test="${command.rptType eq 'total'}">
								<div id="printArea">
									<form method="post" name="form2" id="form2" style="margin: 0;">
										<table class="fxdTable" width="450px">
											<tr class="titleRow">
												...
												<td colspan="4" width="150px;" align="center">
													<%-- 人數 --%>
													<fmt:message key="F0505.table.count" />
												</td>
											</tr>
											<tr class="titleRow">
												<td width="150px;" align="center">
													<%-- 啟用交易 --%>
													<fmt:message key="F0505.table.cnt00" />
												</td>
												<td width="150px;" align="center">
													<%-- 暫停交易 --%>
													<fmt:message key="F0505.table.cnt01" />
												</td>
												<td width="150px;" align="center">
													<%-- 終止交易 --%>
													<fmt:message key="F0505.table.cnt02" />
												</td>
												<td width="150px;" align="center">
													<%-- 帳號總數 --%>
													<fmt:message key="F0505.table.cntTotl" />
												</td>
											</tr>
										...
										<input type="hidden" name="templateName" value="ACL-S-0505-1" />
									</form>
								</div>
								<!-- Button area ------------------------------------------------------------------------>
								<div class="btnContent">
							...
							<c:if test="${command.rptType eq 'monthly'}">
								<div id="printArea">
									<form method="post" name="form2" id="form2" style="margin: 0;">
										<table class="fxdTable" width="600px">
											<tr class="titleRow">
												<td rowspan="2" width="150px;" align="center">
													<%-- 統計月份 --%>
													<fmt:message key="F0505.table.monthly" />
												</td>
												<td colspan="4" width="150px;" align="center">
													<%-- 筆數 --%>
													<fmt:message key="F0505.table.count" />
										...
										<input type="hidden" name="endDate" value="${command.endDate}" />
										<input type="hidden" name="rptType" value="${command.rptType}" />
										<input type="hidden" name="templateName" value="ACL-S-0505-2" />
									</form>
								</div>
								<!-- Button area ------------------------------------------------------------------------>
								<div class="btnContent">
									...
									<form method="post" name="form2" id="form2" style="margin: 0;">
										<table class="fxdTable" width="600px">
											<tr class="titleRow">
												<td rowspan="2" width="150px;" align="center">
													<%-- 統計日期 --%>
													<fmt:message key="F0505.table.daily" />
												</td>
												<td colspan="4" width="150px;" align="center">
													<%-- 筆數 --%>
													<fmt:message key="F0505.table.count" />
										...
										<input type="hidden" name="endDate" value="${command.endDate}" />
										<input type="hidden" name="rptType" value="${command.rptType}" />
										<input type="hidden" name="templateName" value="ACL-S-0505-3" />
									</form>
								</div>
								<!-- Button area ------------------------------------------------------------------------>
								<div class="btnContent">
